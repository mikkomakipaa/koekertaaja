...
### Historical Note: `/api/question-sets/submit`

The direct import endpoint `/api/question-sets/submit` has been removed from the active app surface.
Its former implementation is preserved in:

- `backups/removed-routes/question-sets-submit/route.ts.backup`

If a future import workflow needs to restore it, use the backup rather than this API schema section.
...
curl -X PATCH https://your-domain.com/api/question-sets \
...
The previous cURL example for `/api/question-sets/submit` has been removed because the endpoint is no longer active.
