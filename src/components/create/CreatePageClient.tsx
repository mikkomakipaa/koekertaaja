...
import { Difficulty, Question, QuestionSet, QuestionSetWithQuestions } from '@/types';
...
type TopicStepType = 'quiz' | 'flashcard';
...
const TOPICS_STEP_ID = 'topics';
const QUIZ_TOPIC_STEP_PREFIX = 'topic-';
const FLASHCARD_TOPIC_STEP_PREFIX = 'flashcard-topic-';
...
function formatSchoolLabel(school: School): string {
  return school.municipality ? `${school.name} (${school.municipality})` : school.name;
}
...
export default function CreatePageClient({
  ...
  const [activeTab, setActiveTab] = useState<'create' | 'extend' | 'manage' | 'test-questions' | 'notifications' | 'metrics' | 'users'>('create');
  ...
  const createFormDisabled = Boolean(schoolMembershipError);
  const showSchoolSelector = allowedSchools.length > 1;
  const selectedSchool = allowedSchools.find((school) => school.id === selectedSchoolId) ?? null;
  const canManageFlags = allowedSchools.length > 0;
  ...
  const selectedProviderLabel =
    ...
    DEFAULT_CREATE_PROVIDER_LABEL;
  const sectionLabelClassName = 'mb-2 block text-base font-semibold text-slate-900 dark:text-slate-100';
  const helperTextClassName = 'mt-2 text-sm text-slate-600 dark:text-slate-400';
  ...
  const getQuizTopicStepId = (topicIndex: number) => `${QUIZ_TOPIC_STEP_PREFIX}${topicIndex}`;
  ...
  const parseTopicStep = (
    ...
    if (stepId.startsWith(FLASHCARD_TOPIC_STEP_PREFIX)) {
      ...
      return Number.isInteger(topicIndex) && topicIndex >= 0
        ? { type: 'flashcard', topicIndex }
        : null;
    }

    if (stepId.startsWith(QUIZ_TOPIC_STEP_PREFIX)) {
      const topicIndex = Number(stepId.slice(QUIZ_TOPIC_STEP_PREFIX.length));
      return Number.isInteger(topicIndex) && topicIndex >= 0
        ? { type: 'quiz', topicIndex }
        : null;
    }
    ...
    if (generationMode === 'quiz' || generationMode === 'both') {
      steps.push(
        ...topics.map((topic, topicIndex) => ({
          id: getQuizTopicStepId(topicIndex),
          label: topic.name,
  ...
  const appendTopicSteps = (topics: EnhancedTopic[]) => {
    const topicSteps = buildTopicSteps(topics);
    setCreationSteps((prev) => {
      const baseSteps = prev.filter((step) => step.id === TOPICS_STEP_ID);
      return [...baseSteps, ...topicSteps];
    });
    return topicSteps;
  };
  ...
  const resetCreationStep = (stepId: string) => {
    setCreationSteps((prev) =>
      prev.map((step) =>
        step.id === stepId
          ? {
  ...
  const cloneGenerationSession = (session: GenerationSession): GenerationSession => ({
    ...session,
    topics: [...session.topics],
    enhancedTopics: session.enhancedTopics.map((topic) => ({
      ...topic,
  ...
  const syncGenerationSession = (session: GenerationSession) => {
    setGenerationSession(cloneGenerationSession(session));
  };

  const upsertGenerationError = (session: GenerationSession, error: GenerationError) => {
    session.results.errors = [
      ...session.results.errors.filter((entry) => entry.stepId !== error.stepId),
      error,
    ];
    syncGenerationSession(session);
  };

  const clearGenerationError = (session: GenerationSession, stepId: RetryableStepId) => {
    session.results.errors = session.results.errors.filter((entry) => entry.stepId !== stepId);
    syncGenerationSession(session);
  };

  const storeTopicResult = (
    session: GenerationSession,
    result: GeneratedTopicSet
  ) => {
    session.results.topicSets = [
      ...session.results.topicSets.filter((entry) => entry.stepId !== result.stepId),
      result,
    ].sort((a, b) => a.topicIndex - b.topicIndex);
    clearGenerationError(session, result.stepId);
  };

  const getGenerationStepIds = (steps: CreationStep[]): RetryableStepId[] =>
    steps
      .map((step) => step.id)
  ...
  const mergeTopicSetsForType = async (
    session: GenerationSession,
    type: TopicStepType
  ): Promise<GeneratedSet | null> => {
    const topicSets = session.results.topicSets
    ...
    if (topicSets.length === 1) {
      return topicSets[0].set;
    }

    const fetchedTopicSets = await Promise.all(
      topicSets.map(async (entry) => fetchQuestionSetWithQuestions(entry.set.code))
    );
    const mergedQuestions = fetchedTopicSets.flatMap((set) => set.questions);
    const mergedSet = await createMergedQuestionSet({
      name: getMergedQuestionSetName(type),
      mode: type,
      questions: mergedQuestions,
    });

    await Promise.all(
      fetchedTopicSets.map(async (set) => {
  ...
  const finalizeGeneration = async (session: GenerationSession) => {
    const allSets = (
      await Promise.all([
        mergeTopicSetsForType(session, 'quiz'),
        mergeTopicSetsForType(session, 'flashcard'),
      ])
  ...
  const buildGenerateSingleFormData = (params: {
    ...
    if (params.focusTopic) {
      formData.append('focusTopic', JSON.stringify(params.focusTopic));
    }

    return formData;
  };

  const callGenerateSingle = async (params: {
    type: TopicStepType;
    questionCount: number;
    topics: string[];
    topicDistribution?: TopicDistributionConfig;
    ...
    const response = await fetch('/api/generate-single', {
      ...
      body: buildGenerateSingleFormData(params),
    });
    ...
    if (!response.ok) {
      const details = Array.isArray(data.details)
        ...
        : typeof data.message === 'string'
          ? data.message
          : '';
      const message = data.error || 'Kysymysten luonti epäonnistui';
      throw new Error(details ? `${message}\n\n${details}` : message);
    }

    if (params.type === 'quiz') {
      const createdSet = Array.isArray(data.questionSets) ? data.questionSets[0] : null;
      if (!createdSet) {
        throw new Error('Kokeen luonti ei palauttanut kysymyssarjaa');
      }
      return createdSet as GeneratedSet;
    }

    if (!data.questionSet) {
      throw new Error('Muistikorttien luonti ei palauttanut kysymyssarjaa');
    }

    return data.questionSet as GeneratedSet;
  };

  const buildExtendQuestionSetFormData = (questionsToAddForCall: number) => {
    const formData = new FormData();
    formData.append('questionSetId', selectedSetToExtend);
    formData.append('questionsToAdd', questionsToAddForCall.toString());
    formData.append('questionCount', questionsToAddForCall.toString());
    appendProviderPreference(formData, providerPreference);
  ...
  const callExtendQuestionSet = async (questionsToAddForCall: number) => {
    const response = await fetch('/api/extend-question-set', {
      method: 'POST',
      headers: withCsrfHeaders(),
      body: buildExtendQuestionSetFormData(questionsToAddForCall),
    });
  ...
  const fetchQuestionSetWithQuestions = async (code: string): Promise<QuestionSetWithQuestions> => {
    const response = await fetch(`/api/question-sets?code=${encodeURIComponent(code)}&includeDrafts=1`, {
      method: 'GET',
      headers: getGenerationHeaders(),
    });
    const data = await response.json();
    ...
    if (!response.ok || !data.data) {
      throw new Error(data.error || 'Kysymyssarjan lataus epäonnistui');
    }

    return data.data as QuestionSetWithQuestions;
  };

  const deleteTemporaryQuestionSet = async (questionSetId: string) => {
    const response = await fetch('/api/delete-question-set', {
      method: 'DELETE',
      headers: withCsrfHeaders({ 'Content-Type': 'application/json', ...(selectedSchoolId ? { 'x-school-id': selectedSchoolId } : {}) }),
      body: JSON.stringify({ questionSetId }),
    });

    if (!response.ok) {
      const data = await response.json().catch(() => null);
      throw new Error(data?.error || 'Väliaikaisen kysymyssarjan poisto epäonnistui');
    }
  };

  const createMergedQuestionSet = async (params: {
    name: string;
    mode: 'quiz' | 'flashcard';
    ...
    const response = await fetch('/api/question-sets', {
      ...
      headers: withCsrfHeaders({
        ...
        ...(selectedSchoolId ? { 'x-school-id': selectedSchoolId } : {}),
      }),
      body: JSON.stringify({
        name: params.name,
        subject,
    ...
    if (!response.ok || !data.questionSet) {
      throw new Error(data.error || 'Yhdistetyn kysymyssarjan tallennus epäonnistui');
    }
  ...
  const runTopicStep = async (
    ...
    const focusTopic = session.enhancedTopics[topicIndex];
    if (!focusTopic) {
      const errorMessage = 'Aihealueen tiedot puuttuvat';
      upsertGenerationError(session, { stepId, mode: type, error: errorMessage });
      updateCreationStep(stepId, 'error', { message: errorMessage });
      return false;
    }
    ...
    updateCreationStep(stepId, 'in_progress', {
      message: `Luodaan aiheen "${focusTopic.name}" ${itemLabel}...`,
    });
    ...
    try {
      ...
      updateCreationStep(stepId, 'completed', {
        ...
        message: `Luotiin ${createdSet.questionCount} ${type === 'flashcard' ? 'korttia' : 'kysymystä'}`,
      });
      return true;
    } catch (error) {
      const baseMessage =
        error instanceof Error
          ? error.message
          : type === 'flashcard'
            ? 'Muistikorttien luonti epäonnistui'
            : 'Kysymysten luonti epäonnistui';
      upsertGenerationError(session, {
        stepId,
        mode: type,
        error: `${focusTopic.name}: ${baseMessage}`,
      });
  ...
  const runGenerationStepsFrom = async (
    startStepId: RetryableStepId,
    sessionOverride?: GenerationSession,
    stepsOverride?: CreationStep[]
  ) => {
    const baseSession = sessionOverride ?? generationSession;
    if (!baseSession) {
      return;
    }

    const session = cloneGenerationSession(baseSession);
    const stepIds = getGenerationStepIds(stepsOverride ?? creationSteps);
    const startIndex = stepIds.indexOf(startStepId);

    if (startIndex === -1) {
      return;
    }

    for (const stepId of stepIds.slice(startIndex)) {
      if (cancelRequestedRef.current) {
        return;
      }

      const topicStep = parseTopicStep(stepId);
      if (!topicStep) {
        continue;
      }

      const success = await runTopicStep(stepId, topicStep.topicIndex, topicStep.type, session);
    ...
    await finalizeGeneration(session);
  };
  ...
  const handleCancelGeneration = async () => {
    ...
    if (session && session.results.topicSets.length > 0) {
      await Promise.allSettled(
        session.results.topicSets
          .filter((entry) => Boolean(entry.set.id))
          .map((entry) => deleteTemporaryQuestionSet(entry.set.id!))
      );
    }
  ...
  const handleStepRetry = async (stepId: string) => {
    if (stepId === TOPICS_STEP_ID) {
      return;
    }

    const retryStepId = stepId as RetryableStepId;
    const session = generationSession;
    if (!session) {
      return;
    }

    resetCreationStep(stepId);
    setError('');
    await runGenerationStepsFrom(retryStepId, session, creationSteps);
  };

  const stopCreationAfterTopicFailure = (message: string) => {
    setCreationSteps((prev) =>
      prev.map((step) => {
        if (step.id === TOPICS_STEP_ID) {
          return step;
        }
  ...
  const buildFallbackEnhancedTopics = (topics: string[]): EnhancedTopic[] => {
    if (topics.length === 0) {
      return [];
    }

    const coverage = Number((1 / topics.length).toFixed(2));

    return topics.map((topicName, topicIndex) => ({
      name: topicName,
      coverage:
        topicIndex === topics.length - 1
          ? Math.max(0, 1 - coverage * (topics.length - 1))
          : coverage,
      difficulty: 'normaali',
      keywords: [topicName],
      subtopics: [topicName],
  ...
  const handleSubmit = async (options?: {
    ...
    if (createFormDisabled) {
      setError(schoolMembershipError);
      return;
    }
    ...
    try {
      ...
      if (options?.confirmedTopics) {
        ...
        session.topics = [...options.confirmedTopics];
        ...
        updateCreationStep(TOPICS_STEP_ID, 'completed', {
          count: session.topics.length,
          message: `Käytetään ${session.topics.length} vahvistettua aihealuetta`,
          topics: session.topics,
        });
      } else {
        // Identify topics from material
        updateCreationStep(TOPICS_STEP_ID, 'in_progress', { message: 'Analysoidaan materiaalia...' });
        ...
        try {
          ...
          if (topicsResponse.ok) {
            ...
            if (topics.length === 0) {
              ...
              updateCreationStep(TOPICS_STEP_ID, 'error', {
              ...
              session.enhancedTopics = enhancedAnalysis?.topics
                ? enhancedAnalysis.topics.map((topic) => ({
                    ...topic,
                    keywords: [...topic.keywords],
                    subtopics: [...topic.subtopics],
                  }))
      ...
      const resolvedSteps = [initialSteps[0], ...buildTopicSteps(session.enhancedTopics)];
      const generationStepIds = getGenerationStepIds(resolvedSteps);
      if (generationStepIds.length === 0) {
        await finalizeGeneration(session);
        return;
      }

      await runGenerationStepsFrom(generationStepIds[0], session, resolvedSteps);
    } catch (err) {
  ...
  const tabsConfig: Tab<typeof activeTab>[] = [
    ...
    ...(canManageFlags ? [
      { value: 'notifications' as const, label: 'Ilmoitukset', badge: flaggedQuestions.length },
    ] : []),
    ...(isAdmin ? [
      { value: 'metrics' as const, label: 'Metriikat', adminOnly: true },
      { value: 'users' as const, label: 'Käyttäjät', adminOnly: true },
    ] : [])
  ];
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex shrink-0 items-center gap-2">
              <Button
                asChild
                variant="outline"
                className="h-11 rounded-[14px] border border-black/[0.08] bg-black/[0.02] px-4 text-slate-700 transition-all hover:bg-black/[0.04] hover:text-slate-900 active:scale-[0.98] dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800"
              >
                <Link href="/setup">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-none transition-colors dark:border-slate-800 dark:bg-slate-900 md:p-6">
          {activeTab === 'create' && (
            ...
            <fieldset
              ...
              className={cn('space-y-5', createFormDisabled && 'pointer-events-none opacity-60')}
            >
            <div>
            ...
            {showSchoolSelector ? (
              <div>
                ...
                <p className={helperTextClassName}>
                  Kysymyssarja liitetään valittuun kouluun ja näkyy vain sen koulun pelinäkymässä.
                </p>
              </div>
            ) : selectedSchool ? (
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 shadow-[0_1px_2px_rgba(15,23,42,0.03)] dark:border-slate-700 dark:bg-slate-900">
                <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                  Koulu
                </p>
                <p className="mt-2 flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
                  <GraduationCap weight="duotone" className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />
                  {formatSchoolLabel(selectedSchool)}
                </p>
                <p className="mt-3 text-sm text-slate-600 dark:text-slate-300">
                  Kysymyssarja liitetään automaattisesti tähän kouluun.
                </p>
              </div>
            ) : null}
            ...
            <div className="flex gap-3 pt-3">
              ...
              <Button
                ...
                variant="primary"
                className="flex-1"
                disabled={createFormDisabled || !questionSetName.trim() || !hasSubject || !hasSubjectType || !hasRequiredGrade || !hasExamDate || !hasSelectedSchool || !hasMaterials}
              >
                Luo kysymyssarjat
              </Button>
            </div>
          ...
          {activeTab === 'notifications' && canManageFlags && (
            <div className="space-y-4">
